...
class PIDNet(nn.Module): 
    def __init__(self, m, n, num_classes=19, planes=64, ppm_planes=96, head_planes=128, augment=True):
        ...
        self.lidar_encoder = torch.hub.load('pytorch/vision:v0.10.0', 'resnet18', pretrained=False)
        self.lidar_encoder.conv1 = torch.nn.Conv2d(2, 64, (7, 7), (2, 2), (3, 3), bias=False)


        # For basic blocks and bottlenecks, this is required
        ...
        # self.layer5_lidar_fused_att = FusedLidarAttention(planes * 16, n_head=4, img_anchors=self.img_anchors, lidar_anchors=self.lidar_anchors) 

        # Camera Branch
        # self.layer1_camera_fused_att = CameraCameraAttention(planes, n_head=4, img_anchors=self.img_anchors)
        # self.layer2_camera_fused_att = CameraCameraAttention(planes * 2, n_head=4, img_anchors=self.img_anchors)
        # self.layer3_camera_fused_att = CameraCameraAttention(planes * 4, n_head=4, img_anchors=self.img_anchors)
        # self.layer4_camera_fused_att = CameraCameraAttention(planes * 8, n_head=4, img_anchors=self.img_anchors) 
        # self.layer5_camera_fused_att = CameraCameraAttention(planes * 16, n_head=4, img_anchors=self.img_anchors) 
        # self.layer1_camera_fused_att = FusedCameraAttention(planes, n_head=4, img_anchors=self.img_anchors)
        # self.layer2_camera_fused_att = FusedCameraAttention(planes * 2, n_head=4, img_anchors=self.img_anchors)
        # self.layer3_camera_fused_att = FusedCameraAttention(planes * 4, n_head=4, img_anchors=self.img_anchors)
        # self.layer4_camera_fused_att = FusedCameraAttention(planes * 8, n_head=4, img_anchors=self.img_anchors) 
        ...
        # self.final_layer = SegDecoder(planes * 4, num_classes)
    ...
    def forward(self, image_branch, lidar_branch, fused_branch): 
        ...
        image_branch= self.image_encoder.conv1(image_branch)
        image_branch= self.image_encoder.bn1(image_branch)
        image_branch= self.image_encoder.relu(image_branch)
        image_branch= self.image_encoder.maxpool(image_branch)
        ...
        # lidar_branch = self.lidar_encoder.conv1(lidar_branch)
        # lidar_branch = self.lidar_encoder.bn1(lidar_branch)
        # lidar_branch = self.lidar_encoder.act1(lidar_branch)
        # lidar_branch = self.lidar_encoder.maxpool(lidar_branch)
        # print("Layer 0 sizes: ")
        # print(lidar_branch.shape)
        # print(image_branch.shape)
        # print(fused_branch.shape)

        image_branch = self.image_encoder.layer1(image_branch)
        # lidar_branch = self.lidar_encoder.layer1(lidar_branch)
        fused_branch = self.fused_encoder.layer1(fused_branch)
        # print("Layer 1 sizes: ")
        # print(lidar_branch.shape)
        # print(image_branch.shape)
        # print(fused_branch.shape)
        # lidar_branch, fused_branch = self.layer1_lidar_fused_att(lidar_branch, fused_branch)
        # fused_branch = self.layer1_lidar_fused_att(lidar_branch, fused_branch)
        image_branch, fused_branch = self.layer1_camera_fused_att(image_branch, fused_branch)
        

        image_branch = self.image_encoder.layer2(image_branch)
        # lidar_branch = self.lidar_encoder.layer2(lidar_branch)
        fused_branch = self.fused_encoder.layer2(fused_branch)
        # print("Layer 2 sizes: ")
        # print(lidar_branch.shape)
        # print(image_branch.shape)
        # print(fused_branch.shape)
        # lidar_branch, fused_branch = self.layer2_lidar_fused_att(lidar_branch, fused_branch)
        # fused_branch = self.layer2_lidar_fused_att(lidar_branch, fused_branch)
        image_branch, fused_branch = self.layer2_camera_fused_att(image_branch, fused_branch)

        image_branch = self.image_encoder.layer3(image_branch)
        # lidar_branch = self.lidar_encoder.layer3(lidar_branch)
        fused_branch = self.fused_encoder.layer3(fused_branch)
        # print("Layer 3 sizes: ")
        # print(lidar_branch.shape)
        # print(image_branch.shape)
        # print(fused_branch.shape)
        # lidar_branch, image_branch = self.layer3_lidar_fused_att(lidar_branch, image_branch)
        # lidar_branch, fused_branch = self.layer3_lidar_fused_att(lidar_branch, fused_branch)
        # fused_branch = self.layer3_lidar_fused_att(lidar_branch, fused_branch)
        image_branch, fused_branch = self.layer3_camera_fused_att(image_branch, fused_branch)


        image_branch = self.image_encoder.layer4(image_branch)
        # lidar_branch = self.lidar_encoder.layer4(lidar_branch)
        fused_branch = self.fused_encoder.layer4(fused_branch)
        # print("Layer 4 sizes: ")
        # print(lidar_branch.shape)
        # print(image_branch.shape)
        # print(fused_branch.shape)
        # lidar_branch, image_branch = self.layer4_lidar_fused_att(lidar_branch, image_branch)
        # lidar_branch, fused_branch = self.layer4_lidar_fused_att(lidar_branch, fused_branch)
        # fused_branch = self.layer4_lidar_fused_att(lidar_branch, fused_branch)
